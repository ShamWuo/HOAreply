...
import { listUserHoas, type HoaWithRelations } from "@/lib/hoa";
...
export default async function DashboardPage() {
  ...
  const stats = [
    { label: "Total HOAs", value: hoas.length, helper: "Boards onboarded" },
    { label: "Gmail live", value: connectedCount, helper: "Ready to send" },
    { label: "Awaiting connect", value: pendingCount, helper: "Connect Gmail now" },
  ];

  return (
    <div className="mx-auto flex w-full max-w-6xl flex-col gap-10 px-4 pb-16 pt-12 md:px-6">
      <GlassPanel className="p-8">
        <div className="flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
          <div className="space-y-2">
            <p className="text-xs font-semibold uppercase tracking-[0.4em] text-slate-500">Operator console</p>
            <h1 className="text-4xl font-semibold text-slate-900">Welcome back, {session.user.name ?? session.user.email}</h1>
            <p className="text-base text-slate-700">Connect. Review. Send. Fix issues fast.</p>
          </div>
          <div className="flex flex-wrap items-center gap-3">
            <Link
              href="/app/polls"
              className="rounded-xl border border-slate-200 bg-white px-4 py-2 text-xs font-semibold uppercase tracking-[0.3em] text-slate-700"
            >
              Poll history
            </Link>
            <SignOutButton />
        ...
        {pendingCount > 0 ? (
          <div className="mt-6 flex flex-col gap-3 rounded-2xl border border-amber-200 bg-amber-50 px-5 py-4 text-sm text-amber-900">
            <div className="flex flex-wrap items-center justify-between gap-3">
              <div className="space-y-1">
                <p className="text-base font-semibold">
                  {pendingCount} inbox{pendingCount > 1 ? "es" : ""} idle — connect Gmail to activate AI replies.
                </p>
              ...
              <Link
                ...
                className="inline-flex items-center gap-2 rounded-xl bg-blue-600 px-4 py-2 text-[12px] font-semibold uppercase tracking-[0.25em] text-white"
              >
                Finish connecting →
              </Link>
            </div>
        ...
        <div className="mt-8 grid gap-4 sm:grid-cols-3">
          {stats.map((stat) => (
            <GlassPanel key={stat.label} className="flex min-h-[140px] flex-col justify-between px-5 py-5 text-slate-700">
          ...
          <div className="mt-6 space-y-4">
            {hoas.length === 0 ? (
              ...
              hoas.map((hoa) => (
                <div
                  key={hoa.id}
                  className="flex flex-col gap-3 rounded-xl border border-slate-200 bg-white p-5 text-sm text-slate-600 sm:flex-row sm:items-center sm:justify-between"
                >
                  <div className="space-y-1">
                    ...
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      <span className="text-[11px] uppercase tracking-[0.3em] text-slate-400">ID {hoa.id.slice(0, 6)}</span>
                    </div>
                    <div className="text-xs text-slate-500">
                      {hoa.gmailAccount ? (
                        ...
                        <span className="text-[11px] font-semibold uppercase tracking-[0.3em] text-amber-700">Connect Gmail to start sync</span>
                      )}
                    </div>
                  </div>
                  ...
                  {hoa.gmailAccount ? (
                    ...
                    <div className="flex flex-col gap-2 sm:items-end sm:text-right">
                      <Link
                        href={`/connect/gmail?hoaId=${hoa.id}`}
                        className="inline-flex items-center gap-2 rounded-md border border-[var(--color-border)] bg-white px-3 py-2 text-[12px] font-semibold text-[var(--color-ink)]"
                      >
                        Connect Gmail
                      </Link>
